...
function recordBrowserEvent(
  ...
  if (window.location.pathname.startsWith("/redirecmysanad")) return;
...
export function trackPurchase(
  ...
  email?: string,
  phone?: string,
  items?: Array<{ productId: string; quantity: number; offerPrice: number }>
) {
  if (!ENABLED) return;
  recordBrowserEvent("Purchase", purchaseEventId, { value });
  
  // Format phone to international format for pixel tracking
  const formattedPhone = formatPhoneForPixel(phone);
  
  log("Purchase", orderId, purchaseEventId, value, email, formattedPhone);

  // TikTok requires contents array format
  const ttqContents = items
    ? items.map((item) => ({
  ...
  // TikTok Pixel — contents array + phone_number (not phone)
  // Store purchase data globally so the Event Builder patch can use it
  if (typeof window !== "undefined") {
    window._sanadLastPurchaseValue = value;
    window._sanadLastPurchaseContents = ttqContents;
  }
  ...
  window.snaptr?.("track", "PURCHASE", { 
    price: value, 
    order_id: orderId, 
    client_dedup_id: purchaseEventId,
    ...(email && { email }),
    ...(formattedPhone && { phone: formattedPhone })
  });
}
